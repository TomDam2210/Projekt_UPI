...
namespace ProjektUpi
{
    ...
    {
        SqlConnection VezaNaBazu = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Tomo\Desktop\Škola\TRECA_GODINA\UPI_PROJEKT\ProjektUpi\Baza\Database1.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        public void azurirajsobu_brisanje()
        {
            ...
            int sobaid = Convert.ToInt32(txb_rezervacijaID.Text);
        ...
        private void btn_uredi_Click(object sender, EventArgs e)
        {
            ...
            {
                VezaNaBazu.Open();
                string myquery = "UPDATE Rezervacije_Table set Gost = '" + cb_gost.SelectedValue.ToString() + "', Soba ='" + cb_BrojSobe.SelectedValue.ToString() + "', DatumOd ='" + dateIn.Text + "', DatumDo ='" + dateOut.Text + "'where RezervacijaId =" + txb_rezervacijaID.Text + ";";
                SqlCommand cmd = new SqlCommand(myquery, VezaNaBazu);
                cmd.ExecuteNonQuery();
                MessageBox.Show("Rezervacija uspješno uređena!");
                VezaNaBazu.Close();
